...
# Load data
csv_file = "C:/Users/aarad/OneDrive/Desktop/dctweek1/walkrun.csv"  # <-- Your correct path
if not os.path.exists(csv_file):
    print(f"Error: File not found: {csv_file}")
    exit()

df = pd.read_csv(csv_file)
print("CSV columns:", df.columns.tolist())  # Optional: See column names in terminal
...
app.layout = html.Div([
    ...
    dash_table.DataTable(
        id='stats-table',
        columns=[{'name': col, 'id': col} for col in ['Stat', 'gyro_x', 'gyro_y', 'gyro_z']]
    )
])
